...
export default function App() {
  ...
  const speechDefaults = { enabled: true, focusEnabled: true, rate: 1, voiceURI: '' };
  const [speechPreferences, setSpeechPreferences] = useStored('dormio-speech', speechDefaults, value => ({ ...speechDefaults, ...value }));
  const speech = useReadAloud(speechPreferences);
  const [profile, setProfile] = useStored('dormio-profile', { name: 'Alex W.', campus: 'Virginia Tech', year: 'Sophomore', bio: '', pickup: '', language: 'en', currency: 'USD', avatar: figmaImage('v13_27') }, value => ({ ...value, campus: 'Virginia Tech', language: value.language || 'en', currency: value.currency || 'USD', avatar: value.avatar || figmaImage('v13_27') }));
  ...
  const [exchangeRate, setExchangeRate] = useState({ quote: 'USD', rate: 1 });
  ...
  const money = value => {
    const currency = exchangeRate.quote === profile.currency ? profile.currency : 'USD';
    const rate = currency === profile.currency ? exchangeRate.rate : 1;
  ...
  useEffect(() => {
    if (!currentUser) return;
    let active = true;
    getExchangeRate(profile.currency || 'USD').then(result => {
      if (active) setExchangeRate(result);
    }).catch(() => {
      if (active) setExchangeRate({ quote: 'USD', rate: 1 });
    });
    return () => { active = false; };
  }, [currentUser?.id, profile.currency]);
  ...
  return <div className="dorm-app">
    ...
    <main className={['Explore', 'Saved items', 'My listings'].includes(page) ? 'dorm-main' : 'dorm-main dorm-inner-page'}>
      {['Explore', 'Saved items', 'My listings'].includes(page) && <Marketplace {...{ page, category, search, filtered, saved, toggleSave, openDetail, openSell, setModal, sort, setSort, maxPrice, setMaxPrice, condition, setCondition, onCategory }} formatMoney={money} />}
        {page === 'My wallet' && <CampusWallet {...{ balance, profile, transactions, preferences, setPreferences, setModal, setSetupMethod, navigate, walletAccount, walletConnectionError }} formatMoney={money} cancelReservation={id => {
